...
Traditional elasticity mechanisms and autoscaling frameworks address these challenges by scaling system resources. While effective in centralized cloud environments with abundant resources, such strategies become impractical in edge computing, where additional computational capacity often cannot be provisioned dynamically. Instead, the focus must shift toward scaling the stream processing services themselves. Inspired by Sedlak et al.  Inspired by Sedlak et al. \cite{sedlak_towards_2025}, this thesis adopts a multi-dimensional elasticity approach that adapts the quality and processing standards of the data stream, rather than the underlying infrastructure.
