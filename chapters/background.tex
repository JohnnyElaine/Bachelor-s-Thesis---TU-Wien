...
Agents cannot minimize surprise directly, since the true generative process that produces sensory data
is unknown. Thus, agents aim to minimize an upper bound known as (variational) Free Energy (FE). It captures the difference between the systems internal representation of the world and its observations, making it a computationally feasible approximation for surprise.
...
\begin{enumerate}
  \item Epistemic Value (Information Gain) — Drives exploration by favoring actions that are expected to yield informative outcomes, improving the agent’s generative model.
  \item Pragmatic Value — Drives the agent to select actions that are likely to fulfill its explicit goals or preferences, i.e., it promotes exploitation.
\end{enumerate}
...
Active inference (AIF) operationalizes the free energy principle in artificial agents, extending it to encompass both state estimation and action selection \cite{lanillos_active_2021}.

\section{The Active Inference Cycle}
