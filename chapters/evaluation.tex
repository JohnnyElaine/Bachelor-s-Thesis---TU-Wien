...
In the base case scenario, the AIF agent outperformed the heuristic agent on all relevant SLO metrics. It achieved a \textbf{percentage of full SLO satisfaction of 98.2\%}, compared to \textbf{95.59\%} for the heuristic---a relative improvement of \textbf{+2.73\%}. Similarly, the \textbf{average SLO fulfillment rate} increased from \textbf{98.71\%} to \textbf{99.38\%}, and the \textbf{average overall stream quality score} improved from \textbf{0.7577} to \textbf{0.8007}, a relative increase of \textbf{+5.68%}.

Despite slightly longer maximum violation durations (AIF: 16 timesteps, Heuristic: 14), a relative increase of \textbf{+14.29\%}, the AIF agent demonstrated better convergence behavior, reaching a stable configuration in just \textbf{1.0 timestep}, which is \textbf{+14.29\%} longer than the heuristic baseline. This supports the observation that the AIF agent front-loads exploratory adaptation and quickly stabilizes its belief about optimal stream parameters.
...
\subsection{Variable Computational Budget}

In this more dynamic setting, where the available computational resources change over time, the AIF agent maintained high performance. It achieved \textbf{96.15\%} simultaneous SLO satisfaction compared to \textbf{88.43\%} for the heuristic---a \textbf{+7.72\%} advantage. The \textbf{average SLO fulfillment rate} also improved from \textbf{96.75\%} to \textbf{98.27\%}.

However, the cost of the AIF agent’s equilibrium-seeking behavior is visible in the \textbf{maximum consecutive SLO violation streak}, which rose from \textbf{18} (heuristic) to \textbf{31} (AIF), a \textbf{+72.22\%} increase. This indicates a slower recovery from extreme events where resource availability drops sharply. Nevertheless, once recovery occurs, the AIF agent stabilizes more effectively and avoids repeated violations.

The \textbf{stream quality score} for the AIF agent decreased to \textbf{0.6077}, compared to \textbf{0.7541} for the heuristic, a drop of \textbf{--19.40\%}. This is a direct consequence of the AIF agent favoring SLO compliance over QoE maximization when resource constraints are tight.

\subsection{Variable Computational Demand}
